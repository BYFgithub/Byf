...
def calcScaleZeroPoint(min_val, max_val, num_bits=8):  # 实现量化的基本公式
    qmin = 0.
    qmax = 2. ** num_bits - 1.
...
def quantize_tensor(x, scale, zero_point, num_bits=8, signed=False):
    ...
    return q_x    # 由于pytorch不支持int类型的运算，因此还是用float来表示整数


def dequantize_tensor(q_x, scale, zero_point):    #反量化
...
def search(M):
 ...
 #在训练后量化过程中，需要先统计样本以及中间层的 min、max，同时也频繁涉及到一些量化、反量化操作，因此把这些功能都封装成一个类
class QParam(nn.Module):
    ...
    def update(self, tensor):          #统计 min、max
...
class QModule(nn.Module):
    ...
    def __init__(self, qi=True, qo=True, num_bits=8):      #指定量化的位数和指定是否提供量化输入 (qi) 及输出参数 (qo)
    ...
    def freeze(self):    #在统计完 min、max 后发挥作用,提前计算一些项，同时也将网络的权重由浮点实数转化为定点整数
        pass
...
class QConv2d(QModule):    #卷积层的实现

    def __init__(self, conv_module, qi=True, qo=True, num_bits=8):      #conv_module 模块对应全精度的卷积层，qw 参数用来统计 weight 的 min、max 以及对 weight 进行量化
        super(QConv2d, self).__init__(qi=qi, qo=qo, num_bits=num_bits)
        self.num_bits = num_bits
        self.conv_module = conv_module
        self.qw = QParam(num_bits=num_bits)

    def freeze(self, qi=None, qo=None):   #计算公式中的参数
...
class QReLU(QModule):   #RELU激活
    ...
    def forward(self, x):
        if hasattr(self, 'qi'):
            self.qi.update(x)
            x = FakeQuantize.apply(x, self.qi)   #伪量化
...
class QMaxPooling2d(QModule):     #最大池化
...
class QConvBNReLU(QModule):
    ...
    def freeze(self, qi=None, qo=None):     #在统计完 min、max 后对一些变量进行固化
        if hasattr(self, 'qi') and qi is not None:
            raise ValueError('qi has been provided in init function.')
        if not hasattr(self, 'qi') and qi is None:
            raise ValueError('qi is not existed, should be provided.')
    ...
    def quantize_inference(self, x):     #逐个量化每个模块
        x = x - self.qi.zero_point
        x = self.conv_module(x)
        x = self.M * x
        x.round_()
