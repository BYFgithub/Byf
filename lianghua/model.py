...
class Net(nn.Module):
    ...
    def quantize(self, num_bits=8):   #逐个量化每个模块
    ...
    def quantize_forward(self, x):   #统计 min、max，同时模拟量化误差
        x = self.qconv1(x)
        x = self.qrelu1(x)
    ...
    def freeze(self):     #在统计完 min、max 后对一些变量进行固化
        self.qconv1.freeze()
    ...
    def quantize_inference(self, x):    #实际 inference 的时候用到的函数
...
class NetBN(nn.Module):    #融合BN
    ...
    def quantize(self, num_bits=8):       #量化模块
